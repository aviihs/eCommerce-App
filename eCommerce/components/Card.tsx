...
const Card = ({
  ...
  return (
    <View className="w-44 bg-white rounded-xl shadow-md mr-3 overflow-hidden">
      ...
      <View className="p-2">
        ...
        <Text className="text-gray-500 text-xs mt-1" numberOfLines={2}>{desc}</Text>

        <View className="flex-row items-center mt-2">
          <Text className="text-black font-bold text-sm">Rs {price}</Text>
          {oldPrice && <Text className="text-red-400 text-xs line-through ml-2">Rs {oldPrice}</Text>}
        </View>

        <View className="flex-row items-center justify-between mt-2">
          {discount && <Text className="text-red-500 text-xs">{discount}% OFF</Text>}
